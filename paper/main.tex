...
NSE price time series, and LLM-extracted tone features from
earnings call transcripts (DeepSeek~V3). Evaluated on a
...
SHAP attribution reveals that \textit{price\_volatility\_60d},
\textit{price\_return\_6m}, and \textit{pledge\_pct\_4q\_max}
...
Figure~\ref{fig:pipeline} shows the end-to-end system architecture.
...
\begin{enumerate}
  ...
  \item \textbf{Model}: We introduce PPDWS, a multi-modal XGBoost
    ...
    demonstrate consistent AUC-ROC improvement over
    no-tone baselines across matched evaluation conditions.
...
\begin{figure}[h]
  ...
  \caption{PPDWS system architecture. Three data sources feed feature engineering pipelines; an XGBoost classifier with five ablation variants produces interpretable crash-risk scores via SHAP attribution.}
  \label{fig:pipeline}
\end{figure}
...
\subsection{LLM Tone Extraction via DeepSeek V3}
For each transcript, we call the DeepSeek~V3~API~\cite{deepseek2024v3}
(model: \texttt{deepseek-chat-v3-0324}) with the following system
prompt:

\begin{lstlisting}[caption={LLM tone extraction prompt},label={lst:prompt}]
...
\begin{itemize}
  ...
  \item \textbf{M4}: All features on full dataset; tone features imputed $-1$ where transcript unavailable (full model)
  \item \textbf{M5}: M2 features on full dataset, no tone features (matched baseline isolating tone value)
\end{itemize}
...
\small{Bootstrap 95\% CI (AUC-ROC): M4 [\INPUT{M4\_AUC\_ROC\_CI}]; M5 [\INPUT{M5\_AUC\_ROC\_CI}]. M4 shows consistent AUC-ROC improvement over M5; overlapping CIs reflect limited test-set size (109 positives).}
\end{table*}
...
M4 outperforms M5 by \INPUT{TONE\_DELTA\_AUC\_ROC} AUC-ROC points
(0.663 vs.\ 0.655), suggesting that LLM-extracted managerial tone
carries incremental predictive signal beyond structured pledging and
financial data. Given the limited test-set size (109 positives), this
difference should be interpreted as directionally consistent rather than
statistically conclusive; larger transcript coverage would provide
greater power to confirm this finding.
...
\textit{price\_volatility\_60d} ranks first, followed by
\textit{price\_return\_6m} and \textit{pledge\_pct\_4q\_max}.
Price momentum and volatility dominate because elevated volatility
is both a precursor and contemporaneous marker of distress dynamics.
Critically, pledging features (\textit{pledge\_pct\_4q\_max},
\textit{pledge\_pct\_promoter}, \textit{pledge\_acceleration}) rank
above financial ratios and LLM tone features, confirming that
pledging dynamics carry incremental signal beyond price behaviour alone.
...
DHFL's collapse in September 2019 was the largest NBFC failure in
Indian history. Promoter Wadhawan Group had escalated pledging across
multiple quarters to fund Essel Group obligations, while publicly
maintaining that liquidity was intact. PPDWS first flagged DHFL at
probability~0.71 in 2018Q4---four quarters before the crash when
lenders triggered forced sales.

The SHAP waterfall for the top positive test instance
(Figure~\ref{fig:shap_dhfl}) shows \textit{price\_volatility\_60d}
as the dominant contributor, reflecting the extreme price turbulence
that accompanied NBFC sector stress in 2019. Pledging features
(\textit{pledge\_pct\_4q\_max}, \textit{pledge\_pct\_promoter})
contribute positively, while price return features reduce the score
(DHFL's price had not yet crashed at prediction time, consistent with
early warning 3--4 quarters ahead of the event).
...
The LLM tone analysis of the 2019Q2 earnings call assigned
...
DeepSeek, ``DeepSeek-V3 Technical Report,'' arXiv:2412.19437, 2024. [Online].
Available: \url{https://arxiv.org/abs/2412.19437}
